...
exports.install = async function(name, opts) {
  ...
  const npmBin = (await configs.getRc("npm")) || "npm";
  if (npmBin === "yarn") {
    if (nameInfo.fullNameAndVersion) {
      await this.exec(`add ${nameInfo.fullNameAndVersion}`, opts);
    } else {
...
exports.getDocUrl = async function(name, prefix) {
  const nameInfo = pkgname(name, prefix);
  const registryUri = decodeURIComponent(
    trim(await configs.getRc("registry"), "/")
  );
